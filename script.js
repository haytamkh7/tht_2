document.addEventListener("DOMContentLoaded", function() {
	...
	function isValidInput() {
		...
		const phone = sanitizeInput(document.getElementById('user-phone').value);
		const address = sanitizeInput(document.getElementById('user-address').value);
    ...
    if (!phone.match(phoneRegex)) {
        phoneError.textContent = "טלפון לא תקין";
        valid = false;
    } else if (phone.trim() === "") {
        phoneError.textContent = "נדרש טלפון";
        valid = false;
    } else {
		...
		if (address.trim() === "" || address.length < 3) {
			addressError.textContent = "נדרשת כתובת (להכיל לפחות 3 תווים)";
			valid = false;
		} else {
...
<div class="user-modal-content">
    <label>שם<input type="text" id="user-name" required></label><p id="name-error" style="color: red;"></p><br>
    <label>טלפון<input type="text" id="user-phone" required></label><p id="phone-error" style="color: red;"></p><br>
    <label>כתובת מלאה<input type="text" id="user-address" required></label><p id="address-error" style="color: red;"></p><br>
...
`;
	...
	// Your existing code for generating WhatsApp message remains unchanged
	function generateWhatsAppMessage(userName, userPhone, userAddress, userRequests) {
		userName = sanitizeInput(userName);
		userPhone = sanitizeInput(userPhone);
		userAddress = sanitizeInput(userAddress);
		userRequests = sanitizeTextarea(userRequests);
...
`;
	...
	document.getElementById('submit-user-info').addEventListener('click', () => {
		if (!isValidInput()) {
			return; // Error messages are now handled in the isValidInput function itself
		}
		const userName = sanitizeInput(document.getElementById('user-name').value);
		const userPhone = sanitizeInput(document.getElementById('user-phone').value);
		const userAddress = sanitizeInput(document.getElementById('user-address').value);
		const userRequests = sanitizeTextarea(document.getElementById('user-requests').value);

		const message = generateWhatsAppMessage(userName, userPhone, userAddress, userRequests);
		const phoneNumber = window.config.phoneNumber;
